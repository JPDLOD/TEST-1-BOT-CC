...
if is_quiz:
    ...
    try:
        ...
        cid = None
    # CORRECCIÓN: No forzar a 0, respetar el valor original
    if cid is not None:
        # Validar que esté en rango válido
        if 0 <= cid < len(options):
            kwargs["correct_option_id"] = cid
            logger.info(f"Quiz con opción correcta: {cid} (de {len(options)} opciones)")
        else:
            # Si está fuera de rango, usar 0 como fallback seguro
            logger.warning(f"correct_option_id fuera de rango: {cid} para {len(options)} opciones, usando 0")
            kwargs["correct_option_id"] = 0
    else:
        # Si no hay correct_option_id definido, usar 0 por defecto
