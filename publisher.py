...
from telegram import InlineKeyboardMarkup  # (no usamos botones para preview, pero no molesta)
...
from core_utils import safe_sleep

logger = logging.getLogger(__name__)

# ====== Estado Backup / locks / stats ======
...
# ====== publicar ======
async def publicar_rows(context: ContextTypes.DEFAULT_TYPE, *,
    ...
    for mid, _snip, raw in rows:
        ...
        any_success = False
        for dest in targets:
            # Atajo @@@ (preview nativo con enlace en el texto)
            if "shortcut" in data:
                sc = data["shortcut"] or {}
                label = str(sc.get("label", "Abrir"))
                url = str(sc.get("url", "")).strip()
                if not url.startswith("http"):
                    ok, msg = False, None
                else:
                    text_html = f'<a href="{url}">{label}</a>'
                    coro_factory = lambda d=dest, t=text_html: context.bot.send_message(
                        chat_id=d, text=t, parse_mode="HTML", disable_web_page_preview=False
                    )
            ...
            # Encuestas
                ...
                kwargs = dict(base_kwargs);  kwargs["chat_id"] = dest
                coro_factory = lambda k=kwargs: context.bot.send_poll(**k)
                ok, msg = await _send_with_backoff(coro_factory, base_pause=PAUSE)

            # Resto de mensajes: copiar 1:1
            else:
...
async def publicar(context: ContextTypes.DEFAULT_TYPE, *, targets: List[int], mark_as_sent: bool):
    ...
    from . import SCHEDULED_LOCK as _  # silence linters
...
async def publicar_ids(context: ContextTypes.DEFAULT_TYPE, *, ids: List[int],
    ...
    rows = list(con.execute(sql, ids).fetchall())
    con.close()
    if not rows:
        return 0, 0, {t: [] for t in targets}
    return await publicar_rows(context, rows=rows, targets=targets, mark_as_sent=mark_as_sent)
